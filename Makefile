...
REPO_PATH := github.com/projecteru2/resource-extend
...
build: ## Build the resource-gpu and resource-storage plugin binaries
	CGO_ENABLED=0 go build -ldflags "$(GO_LDFLAGS)" -o resource-gpu ./cmd/resource-gpu
	CGO_ENABLED=0 go build -ldflags "$(GO_LDFLAGS)" -o resource-storage ./cmd/resource-storage
...
clean: ## Remove build artifacts, coverage files, and test cache
	rm -f resource-gpu resource-storage
	rm -rf bin/ dist/
	rm -f coverage.out coverage.html coverage.txt
	go clean -testcache

cloc: ## Count lines of code excluding tests (requires cloc)
	cloc --exclude-dir=vendor,dist,bin --not-match-f='_test\.go$$' .

# --- Help ---

help: ## Show this help message
	@echo "resource-extend Makefile targets:"
	@echo ""
